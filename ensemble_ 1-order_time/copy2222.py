...
class SpectralMethods:
    ...
    def source_term_func(t, x, y, random_coff):  # 修
        ...
        return ((1 + random_coff) * (1 - x) ** 3 * x ** 3 * (1 - y) ** 3 * y ** 3 * np.exp(2 * np.pi * t) * 2 * np.pi + \
                random_coff * (1 + random_coff) * np.exp(2 * np.pi * t) * (
                            (1 - y) ** 3 * y ** 3 * (-360 * x ** 2 + 360 * x - 72) +
    ...
    def gauss_legendre_2d_integral(self, f, x_min, x_max, y_min, y_max, n, t, random_coff, p=None, q=None):
        ...
        integral = np.sum(self.wx_gauss[:, None] * self.wy_gauss[None, :] * integral_values) * 0.25 * (
    ...
    def compute_u_coff(self, j, p, q):
        u_coff_jpq = 4 * self.gauss_legendre_2d_integral(self.u_init_times_basis_func,
                                                         self.x_min, self.x_max, self.y_min,
                                                         self.y_max, 20, t=0,
                                                         random_coff=self.random_coff[j],
                                                         p=p, q=q)
        return j, p, q, u_coff_jpq

    def update_u_coff(self, j, p, q, t):
        u_coff_jpq = (1 + self.dt * (self.mean_random_coff - self.random_coff[j]) *
    ...
    def init_time_coff(self):
        # 初始化u_coff
        with Pool() as pool:
            results = pool.starmap(self.compute_u_coff, [(j, p, q) for j in range(len(self.random_coff))
                                                         for p in range(1, self.N + 1) for q in range(1, self.N + 1)])

        for j, p, q, u_coff_jpq in results:
            self.u_coff[j][p - 1][q - 1] = u_coff_jpq

        # 更新u_coff
        for t in tqdm.tqdm(range(1, self.t_steps + 1), desc="Processing"):
            with Pool() as pool:
                results = pool.starmap(self.update_u_coff, [(j, p, q, t) for j in range(len(self.random_coff))
                                                            for p in range(1, self.N + 1) for q in
                                                            range(1, self.N + 1)])
...
if __name__ == "__main__":
    spectral_method = SpectralMethods(x_min=0, x_max=1, y_min=0, y_max=1, N=10, time_len=1, dt=0.01)
    spectral_method.main_run()